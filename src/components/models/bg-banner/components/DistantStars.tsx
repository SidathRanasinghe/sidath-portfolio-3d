...
type DistantStarsProps = {
  count?: number;
};

const DistantStars = ({ count = 300 }: DistantStarsProps) => {
  ...
  const [twinkleSpeeds] = useState(() => {
    ...
    for (let i = 0; i < count; i++) {
      speeds[i] = 0.3 + Math.random() * 0.7; // Slower twinkling for distant stars
    }
    return speeds;
  });
  ...
  useFrame(state => {
    if (ref?.current && pointsRef?.current) {
      ...
      for (let i = 0; i < count; i++) {
        opacityArray[i] = 0.2 + Math.sin(time * twinkleSpeeds[i] + twinkleOffsets[i]) * 0.15;
      }
  ...
  const starMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      uniforms: {
        size: { value: 0.4 },
        color: { value: new THREE.Color("#ccddff") },
      },
      vertexShader: `
